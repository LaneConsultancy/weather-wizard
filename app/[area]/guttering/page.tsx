...
const SERVICE_SLUG = "guttering";

// Only generate pages for the top 5 towns to keep build focused on highest-traffic areas
...
export default async function AreaGutteringPage({ params }: PageProps) {
  const { area: areaSlug } = await params;
  const area = getAreaBySlug(areaSlug);
  if (!area || !TOP_AREA_SLUGS.includes(areaSlug)) notFound();

  const service = getServiceForArea(SERVICE_SLUG, area.name);
  // Pass the base service object (without resolved fields) to components that accept ServiceContent
      ...
      <section className="bg-slate-900 py-12 md:py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-white/70 text-lg mb-8 max-w-xl mx-auto">
            Get a free quote for your guttering in {area.name} and I&apos;ll tell you exactly

import { AlertTriangle, TrendingUp, XCircle } from "lucide-react";
import type { ServiceContent } from "@/lib/content/services";

interface LossAversionStage {
  icon: typeof AlertTriangle;
  timeline: string;
  title: string;
  description: string;
  borderColor: string;
  iconBg: string;
  iconColor: string;
  badgeBg: string;
  costRange: string;
}

// Service-specific cost escalation timelines
const stagesByService: Record<string, LossAversionStage[]> = {
  guttering: [
    {
      icon: AlertTriangle,
      timeline: "Week 1",
      title: "A Blocked Gutter",
      description:
        "Water overflows onto walls instead of draining away. A small blockage or failed joint at this stage costs around \u00a3100\u2013\u00a3200 to sort.",
      borderColor: "border-[#5ba8a0]",
      iconBg: "bg-[#5ba8a0]/10",
      iconColor: "text-[#5ba8a0]",
      badgeBg: "bg-[#5ba8a0]/10 text-[#2d6b64]",
      costRange: "\u00a3100\u2013\u00a3200",
    },
    {
      icon: TrendingUp,
      timeline: "Month 1",
      title: "Damp Sets In",
      description:
        "Persistent water against the wall causes rising damp and render damage. What was a simple clear-out is now \u00a3500\u2013\u00a31,000 to put right.",
      borderColor: "border-copper",
      iconBg: "bg-copper/10",
      iconColor: "text-copper",
      badgeBg: "bg-copper/10 text-[#9a3412]",
      costRange: "\u00a3500\u2013\u00a31,000",
    },
    {
      icon: XCircle,
      timeline: "Month 6",
      title: "Foundation Problems",
      description:
        "Unchecked water pooling near the base of the property risks subsidence. A \u00a3150 repair has become a \u00a32,000\u2013\u00a35,000 structural problem.",
      borderColor: "border-rust",
      iconBg: "bg-rust/10",
      iconColor: "text-rust",
      badgeBg: "bg-rust/10 text-[#7f1d1d]",
      costRange: "\u00a32,000\u2013\u00a35,000",
    },
  ],

  "bird-proofing": [
    {
      icon: AlertTriangle,
      timeline: "Week 1",
      title: "Pigeons Move In",
      description:
        "A pair of pigeons discovers the gap under your solar panels — the perfect sheltered nesting spot. Fitting mesh at this stage costs around \u00a3200\u2013\u00a3400.",
      borderColor: "border-[#5ba8a0]",
      iconBg: "bg-[#5ba8a0]/10",
      iconColor: "text-[#5ba8a0]",
      badgeBg: "bg-[#5ba8a0]/10 text-[#2d6b64]",
      costRange: "\u00a3200\u2013\u00a3400",
    },
    {
      icon: TrendingUp,
      timeline: "Month 1",
      title: "Droppings Accumulate",
      description:
        "Acidic droppings reduce panel efficiency, block drainage channels, and attract more birds. Clearance and proofing now runs to \u00a3500\u2013\u00a31,000.",
      borderColor: "border-copper",
      iconBg: "bg-copper/10",
      iconColor: "text-copper",
      badgeBg: "bg-copper/10 text-[#9a3412]",
      costRange: "\u00a3500\u2013\u00a31,000",
    },
    {
      icon: XCircle,
      timeline: "Month 6",
      title: "Permanent Damage",
      description:
        "Panel coatings corrode, frames pit, and a health hazard develops from accumulated waste. You\u2019re now looking at \u00a32,000\u2013\u00a34,000 including panel assessment.",
      borderColor: "border-rust",
      iconBg: "bg-rust/10",
      iconColor: "text-rust",
      badgeBg: "bg-rust/10 text-[#7f1d1d]",
      costRange: "\u00a32,000\u2013\u00a34,000",
    },
  ],

  "exterior-painting": [
    {
      icon: AlertTriangle,
      timeline: "Year 1",
      title: "Paint Starts Peeling",
      description:
        "Flaking paint on fascias and soffits exposes bare timber to moisture. A fresh coat now costs \u00a3300\u2013\u00a3600 and takes a day\u2019s work.",
      borderColor: "border-[#5ba8a0]",
      iconBg: "bg-[#5ba8a0]/10",
      iconColor: "text-[#5ba8a0]",
      badgeBg: "bg-[#5ba8a0]/10 text-[#2d6b64]",
      costRange: "\u00a3300\u2013\u00a3600",
    },
    {
      icon: TrendingUp,
      timeline: "Year 2",
      title: "Timber Exposed",
      description:
        "Bare wood absorbs water and begins to soften. Fungal rot sets in. The repair bill has climbed to \u00a3800\u2013\u00a31,500 including filling and priming before repainting.",
      borderColor: "border-copper",
      iconBg: "bg-copper/10",
      iconColor: "text-copper",
      badgeBg: "bg-copper/10 text-[#9a3412]",
      costRange: "\u00a3800\u2013\u00a31,500",
    },
    {
      icon: XCircle,
      timeline: "Year 3",
      title: "Replacement Needed",
      description:
        "Rotted fascias and soffits can no longer hold guttering brackets. Full replacement is unavoidable \u2014 now \u00a32,000\u2013\u00a34,000 including scaffolding.",
      borderColor: "border-rust",
      iconBg: "bg-rust/10",
      iconColor: "text-rust",
      badgeBg: "bg-rust/10 text-[#7f1d1d]",
      costRange: "\u00a32,000\u2013\u00a34,000",
    },
  ],
};

interface ServiceLossAversionProps {
  service: ServiceContent;
  locationName?: string;
}

export function ServiceLossAversion({
  service,
  locationName = "Kent",
}: ServiceLossAversionProps) {
  // Fall back to generic roofing stages if no service-specific stages are defined
  const stages = stagesByService[service.slug] ?? stagesByService["guttering"];

  return (
    <section className="bg-cream py-12 md:py-16">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          {/* Section heading */}
          <div className="text-center mb-8 md:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-4">
              A Low-Cost Fix Today Saves Thousands Later
            </h2>
            <p className="text-slate-600 text-lg max-w-2xl mx-auto">
              Most {service.name.toLowerCase()} problems in {locationName} start small and cheap to fix.
              Leave them, and the cost climbs fast. Here&apos;s the real timeline.
            </p>
          </div>

          {/* Timeline cards */}
          <div className="relative">
            {/* Vertical connector line - desktop only */}
            <div
              className="hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-[#5ba8a0] via-copper to-rust"
              aria-hidden="true"
            />

            <div className="space-y-6 md:space-y-8">
              {stages.map((stage, index) => (
                <div key={stage.timeline} className="relative">
                  {/* Timeline dot - desktop */}
                  <div
                    className={`hidden md:flex absolute left-1/2 -translate-x-1/2 top-8 w-4 h-4 rounded-full ${stage.iconBg} border-2 ${stage.borderColor} z-10`}
                    aria-hidden="true"
                  />

                  {/* Card */}
                  <div
                    className={`relative bg-white rounded-xl border-l-4 ${stage.borderColor} shadow-soft p-6 md:p-8 md:w-[calc(50%-2rem)] ${
                      index % 2 === 0 ? "md:mr-auto" : "md:ml-auto"
                    }`}
                  >
                    {/* Timeline badge */}
                    <span
                      className={`inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider rounded-full px-3 py-1 mb-4 ${stage.badgeBg}`}
                    >
                      {stage.timeline}
                    </span>

                    <div className="flex items-start gap-4">
                      <div
                        className={`flex-shrink-0 p-2.5 rounded-lg ${stage.iconBg}`}
                      >
                        <stage.icon className={`h-6 w-6 ${stage.iconColor}`} />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-slate-900 mb-2">
                          {stage.title}
                        </h3>
                        <p className="text-slate-600 leading-relaxed text-sm">
                          {stage.description}
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
